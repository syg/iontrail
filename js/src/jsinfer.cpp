...
TypeCompartment::fixArrayTypeHelper(JSContext *cx, JSObject *obj, Type type)
{
    ...
    if (p) {
         ...
         * triggering asserts.
         */
        Rooted<Type> origType(cx, Type::UndefinedType());
        if (!type.isUnknown())
            origType = type;
...
TypeCompartment::fixArrayType(JSContext *cx, JSObject *obj)
{
    ...
    fixArrayTypeHelper(cx, obj, type);
}

void
types::FixRestArgumentsType(ExclusiveContext *cxArg, HandleObject obj)
{
...
TypeCompartment::fixRestArgumentsType(JSContext *cx, JSObject *obj)
{
    ...
    fixArrayTypeHelper(cx, obj, Type::UnknownType());
}
