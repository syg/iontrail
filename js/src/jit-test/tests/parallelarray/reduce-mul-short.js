...
function testReduce() {
  ...
  // recompilation of f().
  //
  // During the second run, we encounter various bailouts:
  //
  // - Bailout #1 happens because the fill function is still compiled
  //   and has a valid parallelIonScript, so no warmup is
  //   attempted. We bail when we hit the CallGeneric to f, which was
  //   invalidated after Success #1.
  //
  //   At this point the *caller* of f(), reduce(), gets invalidated
  //   because it was the innermost script that aborted.
  //
  // - Bailout #2 happens because now that reduce() got invalidated
  //   and has no parallelIonScript, the fill() function bails on
  //   CallGeneric to it.
  //
  ...
  var results = ["success", "mixed", "mixed", "success", "success"];
  var actual;
