...
var NBody = {
  ...
  init: function init(mode, numBodies) {
    ...
    for (var i = 0; i < numBodies; i++) {
      // [x,y,z]
      initPos[i] = [Math.floor((Math.random()) * 15000) - 20000,
                    Math.floor((Math.random()) * -10000) - 90000,
                    Math.floor((Math.random() - .25) * 15000) - 30000];

      // [x,y,z,x,y,z]
      initVel[i] = [(Math.random() - 0.5) * NBody.Constant.initialVelocity,
                    (Math.random() - 0.5) * (NBody.Constant.initialVelocity * 200),
                    (Math.random() - 0.5) * (NBody.Constant.initialVelocity + 10),
                    (Math.random() - 0.5) * NBody.Constant.initialVelocity,
                    (Math.random() - 0.5) * NBody.Constant.initialVelocity,
                    (Math.random() - .15) * NBody.Constant.initialVelocity];
    }
